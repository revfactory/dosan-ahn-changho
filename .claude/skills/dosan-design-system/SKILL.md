...
### 4-2. 스크롤 연동 등장 효과 — **금지 (v2.5)**
- **스크롤 리빌(콘텐츠 fade/slide 등장)은 전면 폐기한다.** 사용자 피드백 2회(2026-06-07): 절제 조정으로도 "스크롤 방해" 체감이 해소되지 않았다 — 콘텐츠는 항상 즉시 표시가 원칙이다. 독서 사이트에서 스크롤은 독자의 손이다; 손을 붙잡는 효과는 장식이 아니라 방해다.
- 허용되는 모션은 스크롤과 **무관한** 것뿐: ambient 배경(먹번짐), hover/focus 마이크로 인터랙션, 1회성 카운트업(progress가 아닌 뷰 진입 시 1회, reduce 시 즉시 최종값), 읽기 진행 바(콘텐츠를 움직이지 않는 표시기).
- 앵커 이동의 `scroll-behavior: smooth`도 제거 — 즉시 점프가 기본.

### 4-3. 내비게이션·구조
- 헤더: 스크롤 시 불투명 표면(`--paper-bright`)+그림자로 전환되는 스티키 헤더(v2.6 — blur 금지, 위 유리 카드 규칙 참조). **높이·패딩 등 레이아웃 속성의 스크롤 전이 금지(v2.4)**. 토글은 히스테리시스(진입/해제 임계 분리)로 떨림 방지.
...
- **유리 카드(`--glass`):** 정적 패널(필터 바·지도 컨트롤)에만 사용 — 본문 장문 컨테이너 금지(blur 비용·가독성). **스티키 헤더 등 "움직이는 배경 위 고정 표면"에는 backdrop-filter 금지(v2.6)** — 애니메이션 배경의 블러 재계산이 텍스트 깜빡임을 유발한다. 고정 표면은 불투명(--paper-bright)으로.
...
3. **모션 접근성:** `prefers-reduced-motion: reduce`에서 배경 정지·카운트업 즉시를 확인. **스크롤 연동 등장 효과는 존재 자체가 금지(v2.5)** — .is-pre-reveal류 클래스가 발견되면 위반.
4. **JS 폴백:** JS 비활성 상태에서 모든 콘텐츠 가시, 배경은 CSS-only라 그대로 동작.
