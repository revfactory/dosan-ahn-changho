...
이 문서는 **frontend-developer가 픽셀 단위 추측 없이** 페이지를 구현하도록 모든 시각·모션 결정을 수치로 고정한다. "은은한 베이지"가 아니라 `--paper: #F7F3EB`, "적당한 여백"이 아니라 간격 스케일 토큰, "부드러운 등장"이 아니라 `--dur-reveal 600ms --ease-out`이다. 토큰 외부 하드코딩 색/크기/모션은 계약 위반이다.
...
3. **반응은 즉각, 전환은 여유.** hover/focus는 `--dur-micro`(180ms)로 즉각, 리빌·장면 전환은 `--dur-reveal`(600ms)·`--ease-out`으로 여유 있게.
...
| `--reveal-shift` | `16px` | 리빌 시 translateY 시작 오프셋 |
| `--reveal-stagger` | `75ms` | 카드 그룹 리빌 스태거(60~90ms 중앙값) |
